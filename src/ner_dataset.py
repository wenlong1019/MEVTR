...
def get_file(data_dir: str, mode: Union[Split, str]) -> Optional[str]:
    ...
    fp = os.path.join(data_dir, f"*{mode}.txt")
    _fp = glob.glob(fp)
    if len(_fp) == 1:
        return _fp[0]
    elif len(_fp) == 0:
...
def read_examples_from_file(data_dir, mode: Union[Split, str]) -> List[NerInputExample]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for line in f.readlines():
            ...
            if len(tok) == 4:
                ...
                if len(words) != 0:
                    examples.append(NerInputExample(words=words))
                    words = []
        if words:
            examples.append(NerInputExample(words=words))
    return examples
